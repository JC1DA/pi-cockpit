...
Features, in file order: auth/cookie+token helpers → CHAT_PAGE (login+chat, incl. header with live context usage, web `/new`/`/compact`, and the **ask modal**) → server (`askWaiters`/`askUser`/`settleAsk`/`clientCount`, `/ask-answer` route, `stop()` settles pending asks) → ask bridge (`extractAskQuestions`, `buildAskEnvelope`, `AskTuiComponent`) → factory (`session_*` handlers, web-side commands, `usage` tracking, **`tool_call` hook: ask first-answer-wins**).
...
- **Tests**: `selftest.ts` — 109 checks (unit + HTTP protocol + live SSE + fake-pi wiring incl. ask first-wins both directions), `npx tsc` strict-clean. Reviewed by independent reviewer 2026-08-17; fixes applied: space-in-TUI-type-mode, stale-waiter guard in `askUser`, `wrapText` ellipsis, resync closes ask modal, README Files table.

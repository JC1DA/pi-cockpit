...
Features, in file order: auth/cookie+token helpers → CHAT_PAGE (login+chat, incl. header with live context usage, web `/new`/`/compact`/`/model`/`/tree`, the **ask modal**, and markdown rendering for assistant text: page-script `mdInline`/`md` — escape-first, finalize-only, fences matched via `\u0060` so the page stays backtick-free) → server (`askWaiters`/`askUser`/`settleAsk`/`clientCount`, `/ask-answer` route, `stop()` settles pending asks) → ask bridge (`extractAskQuestions`, `buildAskEnvelope`, `AskTuiComponent`) → factory (`session_*` handlers, web-side commands, `usage` tracking, **`tool_call` hook: ask first-answer-wins**).

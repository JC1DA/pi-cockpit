...
Features, in file order: auth/cookie+token helpers → CHAT_PAGE (login+chat, incl. header with live context usage, web `/new`/`/compact`/`/model`, and the **ask modal**) → server (`askWaiters`/`askUser`/`settleAsk`/`clientCount`, `/ask-answer` route, `stop()` settles pending asks) → ask bridge (`extractAskQuestions`, `buildAskEnvelope`, `AskTuiComponent`) → factory (`session_*` handlers, web-side commands, `usage` tracking, **`tool_call` hook: ask first-answer-wins**).
